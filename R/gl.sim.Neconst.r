...
gl.sim.Neconst <- function(ninds, nlocs, mutation_rate=1e-8,verbose=0) {
	...
	allele_freq <- sample(freq_dist$frequency, nlocs, replace=TRUE, prob=freq_dist$density)
	...
	inds<- new("dartR",gen=genmat,  ind.names = 1:ninds, ploidy=rep(2, ninds))
	#now run a simulation with this landscape
